...
def upload_data(
    faculty_name: str, data: list, type: str, sat: str, errors: list
) -> None:
    ...
    try:
        ...
        logging.info(f"Successfully uploaded: {collection}")
        print(f"Successfully uploaded: {collection}")
    except Exception as exc:
        error = f"upload_data: {collection}\n{exc}"
        errors.append(error)
        print_error(error)
    ...
    try:
        ...
        for root, _, files in os.walk("data"):
            ...
            if groups_list and schedule_list:
                upload_data(
                    faculty_name, groups_list, "ТЕСТ Группы", sat, errors=errors
                )
                upload_data(
                    faculty_name, schedule_list, "ТЕСТ Расписание", sat, errors=errors
                )
        return errors
    except Exception as exc:
        error = f"upload: {exc}"
